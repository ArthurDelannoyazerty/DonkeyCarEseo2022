// F&C : Fast & Curious custom version to add image optimization web control
var driveHandler = new function() {
    ...
    var setBindings = function() {
      ...
      $('#TRANSFORM_IMAGE_BRIGHTNESS').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_BRIGHTNESS");
      });
            
      $('#TRANSFORM_IMAGE_CONTRAST').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_CONTRAST");
      });
                  
      $('#TRANSFORM_IMAGE_SOFT_THRESHOLD_LUM').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_SOFT_THRESHOLD_LUM");
      });

      $('#TRANSFORM_IMAGE_BORDERS_MIN_LUM').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_BORDERS_MIN_LUM");
      });
                  
      $('#TRANSFORM_IMAGE_BORDERS_MAX_SAT').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_BORDERS_MAX_SAT");
      });
                  
      $('#TRANSFORM_IMAGE_BORDERS_WEIGHT').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_BORDERS_WEIGHT");
      });
                        
      $('#TRANSFORM_IMAGE_ORIGINAL_WEIGHT').on('change', function () {
        updateFCSettings($(this).val(),"TRANSFORM_IMAGE_ORIGINAL_WEIGHT");
      });
                              
      $('#RACE_M_GREEN').on('change', function () {
        updateFCSettings($(this).val(),"RACE_M_GREEN");
      });
                              
      $('#RACE_M_YELLOW').on('change', function () {
        updateFCSettings($(this).val(),"RACE_M_YELLOW");
      });
    ...
    var updateUI = function() {
      ...
      $('#fc_settings').val(state.fcSettings);
    ...
    var postDrive = function() {
        ...
        data = JSON.stringify({ 'angle': state.tele.user.angle,
                                ...
                                'fcsetting': state.fcSettingName+":"+state.fcSetting})
        state.fcSettingName = ""
        state.fcSetting = ""
        console.log(data)
        // $.post(driveURL, data)
    ...
    var updateFCSettings = function(fcsetting,name){
      if ((isNaN(fcsetting) || (fcsetting == "")) && (!fcsetting.includes("("))){
        //$("#fc")[0].reset();
        alert("Incorrect input format !");
      }
      else
      {
        state.fcSetting = fcsetting;
        state.fcSettingName = name;
        postDrive()
      }
