...
class Tub(object):
    ...
    def __init__(self, base_path, inputs=[], types=[], metadata=[],
        ...
        if not os.path.exists(self.images_base_path):
            ...
            # F&C : Creating original image array folder :
            os.makedirs(self.images_base_path+"_original", exist_ok=True)
    ...
    def write_record(self, record=None):
        ...
        for key, value in record.items():
            if value is None:
                ...
                # F&C : Managing original image array type with a custom path :
                # This function uses a custom image type to differenciate from standard image.
                elif input_type == 'image_array_original':
                    # Handle image array
                    image = Image.fromarray(np.uint8(value))
                    name = Tub._image_file_name(self.manifest.current_index, key).replace("_original", "") # Keep the original image name
                    image_path = os.path.join(self.images_base_path+"_original", name)
                    image.save(image_path)
                    contents[key] = name
