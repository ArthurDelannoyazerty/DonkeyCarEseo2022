...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    #F&C : Initialize the message bus
    fc_messages = ["","","Ready !"]
    V.mem.put(["fc_messages"], fc_messages)

    fc_current_lap = 0
    V.mem.put(["fc_current_lap"], fc_current_lap)

    #F&C : Initialize the ultrasonic result
    fc_ultrasonic_info = []
    V.mem.put(["fc_ultrasonic_info"], fc_ultrasonic_info)

    #F&C : Initialize ThrottleControl Information
    from donkeycar.parts.fc_throttle_control import AccelerationInfo
    fc_currentthrottle_info = AccelerationInfo(0)
    V.mem.put(["fc_currentthrottle_info"], fc_currentthrottle_info)
    V.mem.put(["fc_multiplier"], 1.0)
    ...
    # F&C : A minimal class to keep an original version of the camera image
    class FC_OriginalImage:
        def run(self, image_array):
            return image_array

    V.add(FC_OriginalImage(), inputs=['cam/image_array'], outputs=['cam/image_array_original'])
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        #F&C : Modification to display the transformed image on web server:
        if (cfg.INFORMATION):
            V.add(ctr,
            inputs=['cam/image_array_web', 'tub/num_records'],
            outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'fc_setting'],
            threaded=True)
        else:
            V.add(ctr,
    ...
    #F&C : Adding race management part
    if cfg.RACE_MANAGEMENT:
        from donkeycar.parts.fc_race_manager import FC_RaceManager
        V.add(FC_RaceManager(cfg), inputs=['cam/image_array', 'user/mode', 'fc_messages', 'fc_setting'], outputs=['cam/image_array', 'user/mode', 'fc_messages', 'fc_current_lap'])

    
    #F&C : See if we should even run the Transform Image part.
    class TransformImageCondition:
        def run(self, mode):
            if (mode == "waiting" or mode == "waiting_test"):
                return False
            else:
                return True

    V.add(TransformImageCondition(), inputs=['user/mode'], outputs=['run_imagetransform'])


    #F&C : Adding camera enhancement part
    if cfg.TRANSFORM_IMAGE:
        from donkeycar.parts.fc_transform_image import FC_TransformImage
        V.add(FC_TransformImage(cfg), inputs=['cam/image_array', 'fc_setting'], outputs=['cam/image_array'], run_condition='run_imagetransform')
    ...
    class PilotCondition:
        def run(self, mode):
            if (mode == "user" or mode == "waiting" or mode == "waiting_test"): # F&C : Added race mode support
                return False
            else:
                return True
    ...
    class RecordTracker:
        ...
        def run(self, num_records):
            ...
            if self.last_num_rec_print != num_records or self.force_alert:
                ...
                # F&C : Disabled Record tracking : handled by information part
                #if num_records % 10 == 0:
    ...
    #F&C : Adding ultrasonic sensor part only on pi
    if cfg.ULTRASONIC_SENSOR and not cfg.DONKEY_GYM:
    ...
    #IMU
        ...
        inputs = ['cam/image_array', 'lidar/dist_array']
    elif cfg.ULTRASONIC_SENSOR and model_type == 'tflite_fc' and not cfg.DONKEY_GYM:
        inputs=['cam/image_array', 'ultra/dist_g', 'ultra/dist_c', 'ultra/dist_d',
                            'ultra/appr_g', 'ultra/appr_c', 'ultra/appr_d']
    else:
        inputs=['cam/image_array']
    ...
    if model_path:
        ...
        load_keras_parts = True # F&C : Allow to disable loading of Keras parts if OpenVino is used.
        ...
        #F&C : Load Openvino model
        elif '.xml' in model_path:
            #when we have a .bin extension
            load_keras_parts = False

            from donkeycar.parts.fc_openvino import FC_Openvino
        ...
        if load_keras_parts:
            ...
            #F&C : Adding Openvino part
            V.add(FC_Openvino(cfg.OPENVINO_DEVICE_TYPE,model_path), inputs=['cam/image_array'], outputs=['pilot/angle', 'pilot/throttle'], run_condition='run_pilot')
    ...
    #F&C : Adding throttle part
    if cfg.THROTTLE_CONTROL:
    ...
    #F&C : Adding avoidance strategy from Collision Detection informations
    if cfg.ULTRASONIC_SENSOR and cfg.ULTRASONIC_SENSOR_AVOIDANCE and not cfg.DONKEY_GYM:
    ...
    #F&C : Adding sound part
    if cfg.SOUND:
        from donkeycar.parts.fc_sound import FC_Sound
        V.add(FC_Sound(cfg), inputs=['fc_messages'], outputs=[], threaded=True)

    #F&C : Adding LED thread part
    if cfg.FCLED:
    ...
    V.add(aiLauncher,
        inputs=['user/mode', 'throttle', 'fc_messages'],
        outputs=['throttle', 'fc_messages'])

    if isinstance(ctr, JoystickController):
        ctr.set_button_down_trigger(cfg.AI_LAUNCH_ENABLE_BUTTON, aiLauncher.enable_ai_launch)


    #F&C : Adding information part
    if cfg.INFORMATION:
        from donkeycar.parts.fc_information import FC_Information
        V.add(FC_Information(cfg.INFORMATION_FREQUENCY, cfg.INFORMATION_SHOW_CONSOLE), inputs=['angle', 'throttle', 'tub/num_records', 'user/mode', 'recording', 'cam/image_array', 'cam/image_array_original', 'fc_messages', 'fc_multiplier', 'fc_current_lap', 'fc_currentthrottle_info'], outputs=['information', 'cam/image_array_web', 'fc_battery'], threaded=True)



    # F&C : Don't run if in waiting mode
    class WaitingRunCondition:
        '''
    ...
    V.add(WaitingRunCondition(), inputs=['user/mode'], outputs=['waiting_running'])
    ...
    if cfg.DONKEY_GYM or cfg.DRIVE_TRAIN_TYPE == "MOCK":
        ...
        #F&C : Add for Waveshare PiRacer not pro (on a second i2c component with pwm frequency 1600/60
        throttle_i2caddr = cfg.PCA9685_I2C_ADDR
        throttle_frequency = 60
        if (hasattr(cfg, "PCA9685_I2C_ADDR1")):
            throttle_i2caddr = cfg.PCA9685_I2C_ADDR1
            throttle_frequency = 1600

        throttle_controller = PCA9685(cfg.THROTTLE_CHANNEL, throttle_i2caddr, frequency=throttle_frequency, busnum=cfg.PCA9685_I2C_BUSNUM)
    ...
    #F&C : Adding ultrasonic sensor inputs only on pi
    if cfg.ULTRASONIC_SENSOR and not cfg.DONKEY_GYM:
        inputs += ['ultra/dist_g', 'ultra/dist_c', 'ultra/dist_d',
                            'ultra/appr_g', 'ultra/appr_c', 'ultra/appr_d']
    ...
    # F&C : Adding original image to tub for saving
    if (cfg.TRANSFORM_IMAGE):
        inputs += ['cam/image_array_original']
        types += ['image_array_original']
